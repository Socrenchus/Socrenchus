...
class aTopic(db.Model):
  """
  Stores an assigned topic
  """
  topic = db.ReferenceProperty(Topic, collection_name="assignments")
  questions = db.ListProperty(db.Key)
  user = db.UserProperty(auto_current_user_add = True)

class aQuestion(db.Model):
  """
  Stores an assigned question.

  >>> key = aQuestion(question = Question(value = u'test').put()).put()
  >>> a = aQuestion.get(key)
  ...
  True
  """
  question = db.ReferenceProperty(Question, collection_name = "assigned_questions")
  ...
  def fromQuestion(qid):
    """
    Create an assigned question from a question 
    
    >>> q = Question(value=u'hello').put()
    >>> key = aQuestion.fromQuestion(q.id()).put()
    
    or get it back from a question id.
    
    >>> aQuestion.fromQuestion(q.id()).key() == key
    True
    
    """
    q = Question.get_by_id(qid)
    result = q.assigned_questions.filter('user =', users.User()).get()
    
    if not result:
      result = aQuestion(question = q)
      result.answers = []
  ...
  def submitAnswer(self, answer_string):
    ...
    Try answering it:
    >>> assignment1 = aQuestion.fromQuestion(first_question.id())
    >>> key = assignment1.put()
    >>> len(assignment1.submitAnswer("oooo!")) == 2
    True
    
    Find the assignment:
    >>> aQuestion.all().filter('question =', next_question).get() != None
    True
    ...
    for ans in self.question.answers:
      if ans.value == answer_string:
        ...
        for q in self.answer.connections.fetch(5):
          tmp = aQuestion.fromQuestion(q.target.key().id())
          result.append(tmp)
          tmp.put()
